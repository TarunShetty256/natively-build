...
interface ElectronAPI {
  ...
  onMissingKeys: (callback: (message: string) => void) => () => void
  ...
  onIntelligenceSuggestedAnswer: (callback: (data: { answer: string; question: string; confidence: number; confidenceLevel?: 'high' | 'medium' | 'low' }) => void) => () => void
...
contextBridge.exposeInMainWorld("electronAPI", {
  ...
  onMissingKeys: (callback: (message: string) => void) => {
  ...
  generateAnswer: (input: string, imagePaths?: string[]) => ipcRenderer.invoke("generate-answer", input, imagePaths),
  ...
  onIntelligenceSuggestedAnswer: (callback: (data: { answer: string; question: string; confidence: number; confidenceLevel?: 'high' | 'medium' | 'low' }) => void) => {
    const subscription = (_: any, data: any) => callback(data)
  ...
  onKeybindRegistrationFailed: (callback: (data: { id: string; accelerator: string; reason: string }) => void) => {
    const subscription = (_: any, data: { id: string; accelerator: string; reason: string }) => callback(data)

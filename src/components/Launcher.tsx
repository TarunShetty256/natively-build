...
const formatStartIn = (startTime: string) => {
  const minutesUntil = Math.max(
    0,
    Math.ceil((new Date(startTime).getTime() - Date.now()) / 60000),
  );
  if (minutesUntil >= 60) {
...
const Launcher: React.FC<LauncherProps> = ({
  ...
  const refreshSelectedMeeting = async () => {
    ...
    try {
      const updatedMeeting =
        await window.electronAPI.getMeetingDetails(selectedId);
      if (updatedMeeting) {
        setSelectedMeeting(updatedMeeting);
      }
    } catch (err) {
      console.error(
        "[Launcher] Failed to refresh selected meeting details:",
        err,
      );
    }
  };
  ...
  const getMeetingSummaryPreview = (meeting: Meeting): string => {
    ...
    const keyPoint = meeting.detailedSummary?.keyPoints
      ?.find((point) => !!point?.trim())
      ?.trim();
    if (keyPoint) return keyPoint;

    const actionItem = meeting.detailedSummary?.actionItems
      ?.find((item) => !!item?.trim())
      ?.trim();
  ...
  return (
    <div className="h-full w-full flex flex-col bg-bg-primary text-text-primary font-sans overflow-hidden selection:bg-accent-secondary/30">
      ...
      <header className="relative w-full h-[40px] shrink-0 flex items-center justify-between pl-0 drag-region select-none bg-bg-secondary border-b border-border-subtle z-[200]">
        ...
        {/* Right: Actions */}
        <div
          className={`flex items-center gap-3 no-drag shrink-0 ${isMac ? "mr-1" : ""}`}
        >
          <button
            onClick={() => {
              onOpenSettings();
                ...
                <div className="max-w-4xl mx-auto space-y-6">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div
                        className={`flex items-center gap-3 border rounded-full px-3 py-1.5 min-w-[140px] transition-colors ${isLight ? "bg-bg-elevated border-border-muted shadow-sm" : "bg-[#101011] border-border-muted"}`}
                      >
                        {isDetectable ? (
                          <Ghost
                            size={14}
                        ...
                        <AnimatePresence mode="wait" initial={false}>
                          {isMeetingActive ? (
                            <motion.div
                              ...
                              className="w-[190px] h-[26px] flex items-center justify-center gap-3 rounded-full 
  ...
  border border-emerald-500/20 hover:border-emerald-500/40"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3 h-[198px]">
                    ...
                    nextMeeting ? (
                      <div
                        className={`md:col-span-2 relative group rounded-xl overflow-hidden ${isLight ? "bg-bg-elevated" : "bg-bg-secondary"} flex flex-col shadow-[0_1px_3px_rgba(0,0,0,0.07),0_1px_2px_rgba(0,0,0,0.04)]`}
                      >
                        {/* Header */}
                        <div className="p-5 flex-1 relative z-10">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-[11px] text-text-tertiary">
                              • Starts in {formatStartIn(nextMeeting.startTime)}
                            </span>
                          </div>
                            ...
                            return (
                              ...
                              >
                                <div className="max-w-[65%] min-w-0">
                                  <div
                                    className={`font-medium text-[14px] truncate ${m.title === "Processing..." ? "text-blue-400 italic animate-pulse" : "text-text-primary"}`}
                                  >
                                    {m.title}
                                  </div>
                                  {m.title !== "Processing..." &&
